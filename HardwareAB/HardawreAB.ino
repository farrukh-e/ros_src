...
ros::NodeHandle   nh;
float dist_per_rev = (2 * 3.14 * .03) / 540;
int past_ticks_left, past_ticks_right;
...
void loop(){
  ...
  if (millis()%1000 == 0)
  {
    ...
    vel_right_msg.data = dist_per_rev * (Encoder_1.getPulsePos() - past_ticks_right);
    vel_left_msg.data = dist_per_rev * (Encoder_2.getPulsePos() - past_ticks_left);
    
    vel_right_pub.publish(&vel_right_msg);
    vel_left_pub.publish(&vel_left_msg);

    past_ticks_left = Encoder_2.getPulsePos();
    past_ticks_right = Encoder_1.getPulsePos();
