#pragma once      //like __main__ to include code only once
...
/// \brief Hardware inteface for a robot
class MyRobotHWInterface : public hardware_interface::RobotHW 
{
public:
  MyRobotHWInterface();
  void write() //doen't return a value
  {
  ...
  /**
  * Read encoder values and calculate angular velocity
  */
  ...
  void read(const ros::Duration &period)
  {
    ...
    vel[2] += ang_distance_right / period.toSec();
  }
...
private:
  ...
  bool stop_callback(std_srvs::Empty::Request& /*req*/, std_srvs::Empty::Request& /*res*/){
    running_ = false;
    return true;
}
...
: running_(true)
  ...
  {
    private_nh.param<double>("wheel_diameter", _wheel_diameter, 0.07);
    ...
    for (unsigned int i=0; i < NUM_JOINTS; ++i)
    {
      ...
      hardware_interface::JointStateHandle state_handles(os.str(), &pos[i], &vel[i], &eff[i]);
      jnt_state_interface.registerHandle(state_handles);
    ...
    registerInterface(&jnt_vel_interface);
        ...
        left_wheel_vel_pub_ = nh.advertise<std_msgs::Float32>("my_robot/left_front_wheel_vel", 1);
        right_wheel_vel_pub_ = nh.advertise<std_msgs::Float32>("my_robot/right_front_wheel_vel", 1);

        left_wheel_angle_sub_ = nh.subscribe("my_robot/left_wheel_angle", 1, &MyRobotHWInterface::leftWheelAngleCallback, this);
        right_wheel_angle_sub_ = nh.subscribe("my_robot/righ_wheel_angle", 1, &MyRobotHWInterface::rightWheelAngleCallback, this);
